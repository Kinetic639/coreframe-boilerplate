-- ============================================================
-- 1. SECURITY DEFINER function for slug availability check
--    Bypasses RLS so onboarding users (not yet org members)
--    can check whether a slug is taken.
-- ============================================================
create or replace function public.check_org_slug_available(p_slug text)
  returns boolean
  language sql
  stable
  security definer
...
grant execute on function public.check_org_slug_available(text) to authenticated, anon;

-- ============================================================
-- 2. Trigger: keep organization_profiles.slug in sync
--    whenever organizations.slug is updated.
-- ============================================================
create or replace function public.sync_org_slug_to_profile()
  returns trigger
  language plpgsql
  security definer
  set search_path = ''
...
drop trigger if exists trg_sync_org_slug_to_profile on public.organizations;
create trigger trg_sync_org_slug_to_profile
  after update of slug on public.organizations
  for each row
  execute function public.sync_org_slug_to_profile();

-- ============================================================
-- 3. Patch the onboarding RPC to also set
--    organization_profiles.slug on creation.
-- ============================================================
...
begin
  ...
  if p_plan_id is not null then
    ...
    if v_resolved_plan_id is null then
      raise notice 'create_organization_for_current_user: plan_id % not found, falling back to free', p_plan_id;
    end if;
  end if;
  ...
  if p_slug is not null and trim(p_slug) != '' then
    ...
    v_org_slug := lower(regexp_replace(
      trim(p_name || coalesce(' ' || v_org_name_2, '')),
      '[^a-z0-9]+', '-', 'g'
  ...
  -- Create org profile with name, name_2, AND slug
  insert into public.organization_profiles (organization_id, name, name_2, slug)
  values (v_new_org_id, trim(p_name), v_org_name_2, v_org_slug)
