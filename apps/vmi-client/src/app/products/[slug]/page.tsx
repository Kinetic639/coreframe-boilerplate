  ...
  return (
    <PublicMarketplaceShell>
      <section className="mx-auto max-w-7xl px-4 py-8 sm:px-6 lg:px-8">
        <Link href="/products" className="inline-flex items-center gap-2 text-sm font-black text-blue-700">
        ...
        <div className="mt-6 grid gap-8 lg:grid-cols-[0.9fr_1.1fr]">
          <div className="relative aspect-square overflow-hidden rounded-3xl border border-slate-200 bg-white shadow-sm">
            <Image src={product.imageUrl} alt="" fill priority unoptimized sizes="(min-width: 1024px) 45vw, 100vw" className="object-cover" />
          </div>

          <div className="space-y-6">
            <div>
              <p className="text-xs font-black uppercase tracking-[0.18em] text-blue-700">{product.brand}</p>
              <h1 className="mt-2 font-display text-4xl font-black leading-tight text-slate-950">{product.name}</h1>
              <p className="mt-4 text-sm leading-7 text-slate-600">{product.description}</p>
            </div>

            <div className="grid gap-3 sm:grid-cols-3">
              <div className="rounded-2xl bg-white p-4 ring-1 ring-slate-200">
                <p className="text-xs font-bold text-slate-500">Cena</p>
                <p className="mt-1 font-display text-xl font-black text-slate-950">
                  {formatPrice(product.priceValue, product.priceMax)}
                </p>
              </div>
              <div className="rounded-2xl bg-white p-4 ring-1 ring-slate-200">
                <p className="text-xs font-bold text-slate-500">Dostępność</p>
                <p className="mt-1 text-sm font-black text-slate-950">{product.availability}</p>
              </div>
              <div className="rounded-2xl bg-white p-4 ring-1 ring-slate-200">
            ...
            <div className="flex flex-wrap gap-2">
              ...
              <Link href={`/products?category=${encodeURIComponent(product.category)}`} className="rounded-full bg-slate-100 px-3 py-1 text-xs font-black text-slate-600">
                {product.category}
              </Link>
            </div>

            <button className="inline-flex items-center gap-2 rounded-2xl bg-blue-700 px-5 py-3 text-sm font-black text-white shadow-sm hover:bg-blue-800">
        ...
        <div className="mt-8 grid gap-6 lg:grid-cols-[1fr_360px]">
          <div className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
            <h2 className="font-display text-xl font-black text-slate-950">Specyfikacja</h2>
        ...
        <div className="mt-10 space-y-4">
          <h2 className="font-display text-xl font-black text-slate-950">Podobne produkty</h2>
          <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
            {related.map((item) => (
              <ProductCard key={item.id} product={item} supplier={snapshot.data.suppliers.find((supplierItem) => supplierItem.id === item.vendorId)} />
            ))}
          </div>
        </div>
      </section>
