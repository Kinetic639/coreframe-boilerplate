...
export async function generateMetadata({ params }: FlyerDetailPageProps): Promise<Metadata> {
  const { slug } = await params;
  const flyerResult = await PublicMarketplaceRepository.getFlyerBySlug(slug);

  if (!flyerResult.success) {
    return {
      title: "Gazetka VMI | Ambra System",
  ...
  return {
    title: `${flyer.title} | Gazetka VMI | Ambra System`,
    description: `Publiczna gazetka VMI ważna od ${flyer.validFrom} do ${flyer.validTo}.`,
    alternates: {
      canonical: `/vmi/flyers/${flyer.slug}`,
    },
    openGraph: {
      title: `${flyer.title} | Gazetka VMI | Ambra System`,
      description: `Publiczna gazetka VMI ważna od ${flyer.validFrom} do ${flyer.validTo}.`,
      url: `/vmi/flyers/${flyer.slug}`,
      images: flyer.coverUrl ? [{ url: flyer.coverUrl }] : undefined,
    },
  };
}
...
export default async function FlyerDetailPage({ params }: FlyerDetailPageProps) {
  ...
  if (!flyerResult.success) notFound();
      ...
      <section className="border-b border-border bg-background">
        <div className="mx-auto grid max-w-7xl gap-8 px-4 py-8 sm:px-6 lg:grid-cols-[0.85fr_1.15fr] lg:px-8">
          ...
          <div className="flex flex-col justify-center space-y-5">
            ...
            {supplier ? (
              <Link
                href={`/vmi/vendors/${supplier.slug}`}
