...
function formatPrice(product: PublicProductDto): string {
  ...
  if (product.priceMode === "from" && product.priceValue)
    return `Od ${product.priceValue.toFixed(2)} PLN`;
  ...
  return (
    ...
    >
      <div className="relative aspect-[4/3] bg-secondary">
        ...
        />
        {product.isNew ? (
          <span className="absolute left-3 top-3 rounded-full bg-amber-700 px-2 py-1 text-[10px] font-black uppercase text-white">
            Nowość
          </span>
        ) : null}
      </div>
      ...
      <div className="flex flex-1 flex-col gap-3 p-4">
        <div>
          ...
          <h3 className="mt-1 line-clamp-2 font-display text-sm font-black text-foreground group-hover:text-amber-700">
            {product.name}
          </h3>
        </div>
        ...
        <div className="mt-auto space-y-3">
          ...
          <div className="flex items-end justify-between gap-3">
            <div>
              <p className="text-sm font-black text-foreground">{formatPrice(product)}</p>
