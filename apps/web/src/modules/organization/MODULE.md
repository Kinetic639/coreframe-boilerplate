...
- **Implementation:** ✅ done
- **Last updated:** 2026-03-05 (Security & Enterprise Hardening pass: Fix A — RESTRICTIVE SELECT policy on `organization_members` using `has_any_org_role`; Fix B — RESTRICTIVE UPDATE policy on `roles` with hardened WITH CHECK + `protect_roles_immutable_columns` BEFORE UPDATE trigger; Fix C — legacy `organization-management/` directory already removed. +2 migrations, +8 T1 tests. Previous: Phase 4 Enterprise UX Hardening + 100k-ready permission compiler.)
- **Owner:** coreframe
...
| `organization_members`        | **SELECT (PERMISSIVE)**: legacy `"Users can view organization members"` — `(user_id = auth.uid()) OR is_org_creator(org_id) OR has_any_org_role(org_id)`. Pre-V2 policy; kept (additive-only rule). **SELECT (RESTRICTIVE — 2026-03-05)**: `"org_members_select_requires_active_role"` — USING: `has_any_org_role(organization_id)`. Blocks removed users (URA soft-deleted → `has_any_org_role=FALSE`) and org creators whose roles were revoked, regardless of permissive policies. Recursion-safe: queries only `user_role_assignments`. **P1-B (2026-02-27)**: `removeMember()` soft-deletes all `user_role_assignments` on removal. **INSERT**: `"Org creators and owners can add members"` (WITH CHECK: `is_org_creator(org_id) OR has_org_role(org_id, 'org_owner')`). **UPDATE (V2 additive)**: `"members_manage_permission_can_update"` — `is_org_member(organization_id) AND has_permission('members.manage')`.                                                                                                                                                                                                                                                                                                                                                                             |
...
| `roles`                       | **SELECT (PERMISSIVE)**: `is_org_member` (org roles) or global `is_basic=true` (system roles); `deleted_at IS NULL`. **UPDATE (PERMISSIVE)**: `roles_update_permission` — USING: `org_id IS NOT NULL AND is_basic=false AND is_org_member AND has_permission(members.manage) AND deleted_at IS NULL`; WITH CHECK: `org_id IS NOT NULL AND is_basic=false`. **UPDATE (RESTRICTIVE — 2026-03-05)**: `roles_update_restrictive_hardened` — USING: `org_id IS NOT NULL AND is_org_member(org_id)`; WITH CHECK: `org_id IS NOT NULL AND is_org_member(org_id) AND has_permission(org_id, members.manage)`. Tightens WITH CHECK to require membership + permission in the NEW row's org (prevents cross-org writes). **BEFORE UPDATE trigger (2026-03-05)**: `roles_protect_immutable_columns` raises P0001 if `organization_id`, `is_basic`, or `scope_type` changes (IS DISTINCT FROM). **INSERT**: `has_permission(org_id, members.manage) AND is_org_member AND deleted_at IS NULL`. **DELETE**: same.                                                                                                                                                                                                                                                                                                  |
...
Migrations applied between initial 12 (2026-02-27) and Phase 3 that were previously omitted from this list:

- `20260227340000_fix_roles_update_with_check_allow_soft_delete.sql` — Bug fix: `roles_update_permission` WITH CHECK clause now allows soft-delete (setting `deleted_at`) for `members.manage` holders; previously the minimal WITH CHECK blocked soft-deletes.
- `20260227350000_fix_roles_update_with_check_minimal.sql` — Simplification: drops the soft-delete WITH CHECK clause introduced in `20260227340000` in favour of a minimal WITH CHECK `((organization_id IS NOT NULL) AND (is_basic = false))` that prevents system-role mutation while allowing org-role updates/soft-deletes.
- `20260227360000_add_delete_org_role_rpc.sql` — Adds `delete_org_role(role_id, org_id)` RPC function for atomic hard-delete of a role and its `role_permissions` rows; used by `OrgRolesService.deleteRole()` as a server-side atomic alternative to two separate client deletes.
- `20260228100000_add_module_access_permissions.sql` — Inserts `module.*` wildcard and `module.organization-management.access` into `permissions`; grants `module.*` to `org_owner` via `role_permissions`; adds `module.organization-management.access` to `ORG_ONLY_SLUGS` (branch roles cannot hold this perm).
- `20260302100000_add_branch_view_permissions.sql` — Inserts three branch-visibility permissions (`branches.view.any`, `branches.view.update.any`, `branches.view.remove.any`) into `permissions` and grants them to `org_owner`. These control the branch switcher, not org-management CRUD.

100k-ready permission-compiler migrations (applied 2026-03-04, affect this module's permission resolution):

- `20260304110000_drop_redundant_upo_unique_constraint.sql` — Drops old unique constraint on `user_permission_overrides` that became redundant after the 100k schema changes.
- `20260304120000_add_audit_rls_permission_gate_slugs_fn.sql` — Adds `audit_rls_permission_gate_slugs()` RPC for verifying that key permission slugs exist in the `permissions` table; used in migration validation.
- `20260304130000_add_uep_partial_indexes_10k_ready.sql` — Drops old partial indexes `uep_org_exact_active_idx` / `uep_branch_exact_active_idx` (on `permission_slug`) and replaces them with `uep_org_slug_exact_idx` / `uep_branch_slug_exact_idx` (on `permission_slug_exact`). Improves UEP lookup performance at scale.
- `20260304140000_add_audit_uep_partial_indexes_fn.sql` — Adds `audit_uep_partial_indexes()` RPC that verifies the new partial indexes are present; used in CI validation.
- `20260304150000_add_permission_slug_exact_compiler_expansion.sql` — Adds `permission_slug_exact` column to `user_effective_permissions`; updates `compile_user_permissions` to expand wildcard slugs (e.g., `warehouse.*` → concrete rows per registry) and populate `permission_slug_exact`; updates `has_permission`, `has_branch_permission`, `user_has_effective_permission` to match on `permission_slug_exact`; replaces unique constraint with `unique_v3 (user_id, org_id, permission_slug_exact, branch_id)`; backfills all existing UEP rows.
- `20260304151000_fix_compiler_revoke_matches_exact.sql` — Bug fix: `compile_user_permissions` revoke path now deletes rows matching `permission_slug_exact` (not `permission_slug`) to avoid leaving stale concrete-slug rows when a wildcard override is revoked.

Security & Enterprise Hardening (applied 2026-03-05):

- `20260305100000_org_members_select_hardening.sql` — **Fix A**: Adds RESTRICTIVE SELECT policy `"org_members_select_requires_active_role"` on `organization_members`. USING: `has_any_org_role(organization_id)`. Blocks removed users (URA soft-deleted) and org creators whose role was revoked from reading any org member rows. Recursion-safe: `has_any_org_role` queries `user_role_assignments`, not `organization_members`. Legacy permissive policy retained (additive-only).
- `20260305110000_roles_update_hardening.sql` — **Fix B**: (1) Adds RESTRICTIVE UPDATE policy `"roles_update_restrictive_hardened"` on `roles`. USING: `org_id IS NOT NULL AND is_org_member(org_id)`. WITH CHECK: `org_id IS NOT NULL AND is_org_member(org_id) AND has_permission(org_id, 'members.manage')`. Tightens WITH CHECK to require active membership + permission in the NEW org value — prevents cross-org `organization_id` mutations. (2) Adds `protect_roles_immutable_columns()` SECURITY DEFINER BEFORE UPDATE trigger: raises P0001 if `organization_id`, `is_basic`, or `scope_type` changes (IS DISTINCT FROM — NULL-safe). Provides hard DB-level immutability independent of RLS.

Invitation Acceptance Flow (applied 2026-03-06):

- `20260306120000_invitation_acceptance_flow.sql` — (1) Adds `role_id FK → roles.id ON DELETE SET NULL` to `invitations` table (idempotent). (2) Adds partial unique index `invitations_org_pending_email_idx` on `(organization_id, lower(email)) WHERE status='pending' AND deleted_at IS NULL` — prevents duplicate pending invitations per email per org. (3) Creates `accept_invitation_and_join_org(p_token TEXT)` SECURITY DEFINER function (runs as function owner, bypasses RLS): validates token → invitation state → expiry → authenticated user → email match → atomically creates `organization_members` row (ON CONFLICT DO UPDATE reactivates deactivated members) → assigns `org_member` base role via URA (ON CONFLICT DO UPDATE reactivates soft-deleted URA) → assigns invitation `role_id` if present and org-scoped → marks invitation accepted. Returns `{ success, organization_id? }` JSONB. GRANT EXECUTE to `authenticated` only.
...
| `acceptInvitationAction`         | `invitations.ts` | `token: string` (positional)                                                                                             | None (invitee-facing, auth required only)                           | ✅ via SECURITY DEFINER RPC |
...
| 4   | INFO     | Module config (`src/modules/organization/config.ts`) still points to legacy `/dashboard-old/` routes                                                                                                                                                           | ✅ Closed 2026-02-26 — `items` replaced with `[]`, navigation driven by sidebar V2 registry                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                |
