-- IC-5 self-caught regression, live-caught by the FULL regression suite
-- itself (101/104 both broke: "have 20, want 10"): attach_repair_order_
-- line_movement's own new auto-rebuild (previous migration) unconditionally
-- ran even when CALLED FROM WITHIN receive_repair_order_stock, which
-- ALREADY performs its own direct, authoritative UPSERT into repair_
-- order_line_locations immediately after its own call to attach --
-- causing the quantity to be applied TWICE (once by attach's own rebuild,
-- once by receive's own additive UPSERT). Fixed by making attach's own
-- auto-rebuild conditional on the SAME ambra.repair_order_attribution_
-- authoritative GUC receive/putaway already set around their own calls --
-- exactly the existing "defer to the orchestrating caller" signal already
-- established for the ledger-sync trigger. A genuine standalone attach
-- call (the GUC unset) still self-syncs, unchanged; a call from within an
-- already-authoritative orchestrator now correctly defers to it.
...
begin
  ...
  -- IC-5 (corrected): only self-sync when this is a genuine standalone
  -- call. When an authoritative orchestrator (receive_repair_order_stock/
  -- putaway_repair_order_stock) is already managing the projection for
  -- this same transaction, defer to its own direct write entirely --
  -- rebuilding here too would double-apply the same quantity.
