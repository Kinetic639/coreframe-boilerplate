-- IC-5: extends the existing ledger-sync trigger with reversal-
-- awareness, eliminating the IC-2 temporary staleness compromise. The
-- generic inventory engine (inventory_reverse_movement) is NOT touched
-- and remains entirely RepairOrder-agnostic -- all of this logic lives
-- inside this RepairOrder-domain trigger, which already existed and
-- already contained significant domain logic (see architecture doc §18
-- disposition: retained and extended, not replaced -- the coupling here
-- is a generic ledger table read by a domain-specific listener, not the
-- reverse).
--
-- New behavior, checked BEFORE the pre-existing authoritative-GUC
-- short-circuit:
--   - If the firing ledger row belongs to a REVERSAL movement (its own
--     header has original_movement_id IS NOT NULL), the ORIGINAL
--     movement's own corresponding line is found by ordinal position
--     (line_number correlation -- never SKU), its own receipt/issue
--     attribution links are mirrored onto the reversal's own movement
--     line as new 'reversal'-typed links (idempotent), and the affected
--     bucket(s) are rebuilt via the SAME deterministic primitive used
--     for on-demand reconciliation -- rebuild-equals-incremental holds
--     BY CONSTRUCTION for the reversal case, not merely by separate
--     testing.
--   - Otherwise, if authoritative='on' (ordinary putaway/receive
--     suppression), behavior is UNCHANGED: do nothing, trusting the
--     calling RPC's own direct, same-transaction write later in its own
--     body. Rebuilding here would race ahead of that write.
--   - Otherwise (a genuine generic, non-RepairOrder-aware movement),
--     the pre-existing confidence-based inference logic runs UNCHANGED,
...
BEGIN
  ...
  -- IC-5: reversal detection -- checked first, independent of the
  -- authoritative GUC's own value (reversal always sets it, but this
  -- check is what actually decides the branch, not the GUC alone).
    ...
    IF v_original_line_id IS NOT NULL THEN
      FOR v_orig_link IN
        ...
        WHERE inventory_movement_line_id = v_original_line_id
          AND relation_type IN ('receipt', 'issue')
      LOOP
  ...
  IF current_setting('ambra.repair_order_attribution_authoritative', true) = 'on' THEN
    -- Ordinary putaway/receive suppression, unchanged: the calling RPC
    -- writes the projection directly, in this same transaction, AFTER
    -- this trigger fires. Rebuilding here would race ahead of that
    -- write and destroy state the caller is about to correctly use.
    RETURN NULL;
  END IF;
