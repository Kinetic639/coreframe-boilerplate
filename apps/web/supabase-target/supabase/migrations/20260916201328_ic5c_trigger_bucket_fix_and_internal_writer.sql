-- IC-5 CORRECTION PASS -- Finding A (BLOCKER): the reversal branch
-- rebuilt NEW.location_id and movement_line.destination_location_id,
-- both of which are the SAME bucket for a relocation reversal (the only
-- ledger row that fires this trigger for a relocation reversal is the
-- destination-decrease effect -- inventory_reverse_movement preserves
-- source_location_id/destination_location_id verbatim and only inverts
-- each effect's own direction). This left the SOURCE bucket (e.g. the
-- Receiving location after reversing a putaway) never rebuilt, stale.
-- Fixed: buckets are now derived from the REVERSAL MOVEMENT LINE's own
-- source_location_id/destination_location_id columns directly, never
-- from NEW.direction or NEW.location_id. Both are rebuilt (each exactly
-- once, guarded against source=destination).
--
-- Also Finding B: the reversal-mirroring INSERT now goes through the
-- shared write_repair_order_line_movement_link_internal primitive
-- instead of a direct INSERT, matching attach/putaway.
--
-- SUPERSEDED by 20260916201541_ic5c_trigger_reversal_mirror_skip_posted_
-- check.sql (the internal writer's own p_require_posted=true default
-- broke this call site, self-caught live via the exact Finding-A
-- reproduction -- the reversal's own movement line is not yet 'posted'
-- at the moment this trigger fires). Kept as its own forward migration
-- per this project's own never-edit-an-applied-migration discipline.
