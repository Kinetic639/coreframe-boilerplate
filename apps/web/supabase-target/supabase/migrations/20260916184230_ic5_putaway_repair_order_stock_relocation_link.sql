-- IC-5: putaway now records its own relocation as canonical business-
-- attribution history (relation_type='relocation'), one link per line,
-- referencing the SAME 801 movement line the engine call just posted
-- (which carries both the source-decrease and destination-increase
-- effects) -- this is what makes rebuild_repair_order_projection_bucket_
-- internal's own ledger+links join correctly replay a putaway's own
-- physical relocation, closing the gap live-caught while proving
-- rebuild-equals-incremental. Still does NOT go through attach_repair_
-- order_line_movement (its own category-match validation is receipt/
-- issue-specific, not relocation-aware) -- inserted directly, in the
-- same already-privileged SECURITY DEFINER context and same
-- transaction as putaway's own existing direct projection write.
--
-- SELF-CAUGHT DEFECT in THIS version (fixed in the very next migration,
-- before any pgTAP ran): the movement-line lookup below correlates by a
-- field-value match (source/destination/variant/quantity), which is
-- unsafe when two DIFFERENT RepairOrderLines putaway the SAME quantity
-- of the SAME variant to the SAME shelf in one call. Kept here verbatim
-- (never editing an already-applied migration) -- see the immediately
-- following migration for the corrected, ordinal-correlated version.
...
BEGIN
  ...
  LOOP
    ...
    WHERE movement_id = v_movement_id
      AND source_location_id = v_receiving_location_id
      AND destination_location_id = p_destination_location_id
      AND variant_id = v_line_variant_id
      AND quantity = v_qty
    LIMIT 1;
