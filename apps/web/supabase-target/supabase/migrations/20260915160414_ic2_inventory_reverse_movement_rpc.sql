-- IC-2 (Movement Reversal): the new canonical reversal RPC.
--
-- Eligibility (re-checked against the row-locked original, never an
-- earlier unlocked read): status must be 'posted'; original_movement_id
-- must be NULL (a reversal movement cannot itself be reversed -- product
-- decision A); reversal_movement_id must be NULL (a posted original may be
-- reversed at most once -- product decision A). Actor identity is checked
-- first (28000 on mismatch, unrelated to the target's own existence). A
-- non-blank reason is mandatory (22023). A genuinely nonexistent movement
-- and one the actor lacks warehouse.inventory.operate on both raise the
-- SAME P0002 "not found" -- wrong-org/wrong-branch/inaccessible movements
-- must not leak existence via a distinguishable error.
--
-- The reversal is a brand-new movement (type '900'/KOR, see the sibling
-- seeding migration), built by copying each original line's own variant,
-- unit, lot, serial, and source/destination location identity, with each
-- of the original TYPE's own effects inverted (increase<->decrease) and
-- passed explicitly to `inventory_finalize_posting`'s new
-- `p_explicit_effects` parameter -- reusing the exact same canonical
-- engine (same locking, same IC-1 hard invariant, same ledger insert, same
-- document-number allocator), never a second posting engine.
--
-- After the reversal posts, bidirectional linkage is set in the SAME
-- transaction: original.status='reversed', original.reversal_movement_id
-- = the new movement's id (the reversal's own original_movement_id was
-- already set at INSERT time). This is the only mutation to an
-- already-posted header, permitted narrowly because `ambra.inventory_
-- movement_engine = 'on'` is set for this same transaction, matching
-- `inventory_prevent_header_modification`'s own existing, narrowly-scoped
-- allowance -- no trigger is disabled or weakened.
...
EXCEPTION
  WHEN OTHERS THEN
    IF SQLSTATE IN ('28000', '22023', 'P0002', 'P0003', 'P0004', 'P0005', 'P0006') THEN
      RAISE;
    END IF;
