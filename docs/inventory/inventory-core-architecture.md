...
| `inventory_create_and_finalize`/`inventory_create_draft`/public `inventory_finalize_posting` — **CRITICAL, NEW, IC-3-discovered, 2026-09-16, NOT fixed this pass** | **The single most severe finding of this project to date.** All three carry live `anon` EXECUTE and perform **ZERO** actor-identity or permission check of their own — `inventory_create_draft` never checks `auth.uid()`, never calls `has_branch_permission`, relies entirely on RLS, which its own `SECURITY DEFINER`/owner-`postgres` execution context (`rolbypassrls=true`) **bypasses structurally**. Live-proven via a safe, transaction-scoped, rolled-back probe: a **fully unauthenticated** `anon` session — no JWT claims, no `auth.uid()`, no session of any kind — called `inventory_create_and_finalize` directly and posted a real, immutable 999-unit movement to an arbitrary organization/branch it merely named a UUID for. This is a **complete authentication bypass**, strictly worse than the IC-2 explicit-effects P0 (which at minimum required a valid authenticated session) and worse than the header-immutability GUC bypass above (which at minimum required holding a real warehouse permission) — it requires **no account of any kind**. Pre-existing: predates IC-1 entirely; not introduced or worsened by IC-3's own new code (`inventory_receive_stock` and both refactored wrappers each perform their own actor-identity + permission checks BEFORE ever calling down into this unguarded layer — live-verified `anon`/no-permission calls to all three IC-3 entry points are correctly rejected). But the unguarded layer remains directly reachable regardless of anything built on top of it. | **Close in IC-7 as the top-priority item**, ahead of the other rows in this table given its severity (zero authentication required, not merely a privilege-escalation-among-legitimate-users class of issue). Minimum fix: add the same actor-identity (`28000`) + `has_branch_permission` (`42501`) checks every other canonical RPC already has to `inventory_create_draft` itself (the shared root of both `inventory_create_and_finalize` and the public `inventory_finalize_posting`'s own draft-creation callers), then re-verify EVERY existing legitimate caller (all IC-1/IC-2/IC-3 wrappers, `InventoryMovementsService`, `ambra-location-inventory.ts`) still supplies a real actor/permission so the fix does not silently break them — an audit-sized undertaking, correctly out of IC-3's own narrow scope, but the highest-severity item on IC-7's own list.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             |
