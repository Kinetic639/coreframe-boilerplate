...
| `inventory_movement_headers`/`_lines`                                                                                                                                                                                  | **CLOSED, IC-7, 2026-09-17**: was sharpened-but-not-closed after IC-2's own security-boundary pass (the posting-immutability trigger backstopped UPDATE/DELETE only by checking the GUC, never which columns changed). IC-7 removed the GUC entirely as an authorization signal: the trigger now validates the SUBSTANCE of any change to a posted header — only the exact reversal-lifecycle delta is ever permitted (`status: posted→reversed`, `reversal_movement_id`/`reversed_by`/`reversed_at` from NULL to real values, every other column byte-identical to OLD), AND the new `reversal_movement_id` must genuinely double-link back to a real, correctly-typed (`movement_type_code='900'`) row — which cannot be forged, since `900`-type movements cannot be manually created (IC-7A's own closed contract). Lines: no legitimate path ever updates/deletes an existing line post-posting, so the GUC-gated exception was removed entirely (unconditional denial). Also closed the status-blind INSERT gap (see the new row below) in the same phase. | **Keep** — closed, live-verified (`110_ic7_security_write_boundary_test.sql` Scenarios A/B/C/D), full regression green.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 |
| `inventory_movement_headers`/`_lines` — status-blind raw INSERT                                                                                                                                                        | **CLOSED, IC-7, 2026-09-17**: IC-0's own finding (an `operate`-permissioned actor could raw-INSERT a fully fabricated `status='posted'` header) was re-confirmed still live this phase, and found to ALSO extend to lines (a raw INSERT of a new line under an existing, genuinely-posted `movement_id` was never blocked, since the immutability trigger only fires on UPDATE/DELETE).                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          | **Keep** — closed with a RESTRICTIVE `status='draft'`-only INSERT policy on both tables, live-verified.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 |
| `inventory_reservations`/`_lines`, `inventory_allocations`/`_lines`                                                                                                                                                    | **CLOSED, IC-7, 2026-09-17**: IC-0's own 2026-09-15 finding (single PERMISSIVE ALL policy, no RESTRICTIVE ownership-aware policy — confirmed exploitable, any `operate`-permissioned actor could raw-write) was re-confirmed still live this phase (both by live `pg_policies` inspection and an empirical raw-DML probe), then closed with the same RESTRICTIVE `USING(false)/WITH CHECK(false)` pattern IC-4/IC-5 already proved safe. Required converting `inventory_create_reservation`/`inventory_release_reservation`/`inventory_create_allocation`/`inventory_release_allocation` from `SECURITY INVOKER` (undisclosed until this phase) to `SECURITY DEFINER` with actor-identity checks added, since their own INVOKER status meant the new RLS would otherwise have blocked their own legitimate writes too.                                                                                                                                                                                                                                           | **Keep** — closed, live-verified, full reservation→allocation→release chain re-run end to end unaffected.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               |
| `inventory_containers`/`_container_lines`                                                                                                                                                                              | **CLOSED for generic rows, IC-7, 2026-09-17**: RepairOrder-owned rows already RESTRICTIVE-closed (Phase 10C, unchanged). Generic rows were openly PERMISSIVE ALL — this phase confirmed (fresh, live repo-wide grep) that IC-6 already deleted the sole legacy direct-writer (`ambra-location-inventory.ts`'s own 4 write actions), making it safe to close entirely.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            | **Keep** — closed, live-verified, canonical container RPCs (create/add/remove/seal) unaffected.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         |
...
| Function-level `anon` EXECUTE grants (new functions)                                                                                                                                                                   | **DECIDED, IC-7, 2026-09-17 (Option B)**: root cause confirmed via `pg_default_acl` -- `ALTER DEFAULT PRIVILEGES FOR ROLE postgres IN SCHEMA public GRANT EXECUTE ... TO anon` is schema-wide, silently re-applying on every `CREATE OR REPLACE FUNCTION`. A live audit this phase (going further than IC-7A's own deferred investigation) found dozens of genuinely, deliberately public-facing functions across UNRELATED modules (invitation acceptance, signup, org creation) that legitimately rely on this same default -- changing it schema-wide is confirmed UNSAFE within Inventory Core's own narrow scope.                                                                                                                                                                                                                                                                                                                                                                                                                                           | **Keep the default; enforce per-function instead.** Every physical-stock-mutation Inventory Core function was given an explicit `REVOKE ALL FROM PUBLIC, anon` this phase (9 migrations). The new `110_ic7_security_write_boundary_test.sql`'s own grant-privilege assertions (Scenario N) plus the full `function-grant-matrix.md` sweep serve as the enforced, re-runnable regression check going forward -- not "remember to revoke" human discipline alone. A disclosed, not-yet-hardened set remains (product-catalog/procurement/audit-domain functions, out of this phase's own physical-stock-write scope) -- see `docs/inventory/reviews/ic-7-review/function-grant-matrix.md`.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                |
...
## 9E. IC-7 — Inventory Security / Write-Boundary Closure + Module-Boundary Audit

**✅ DONE (2026-09-17)**.

**Goal, restated**: close every remaining raw-write gap architecture
doc §9's own table listed as open, verify the full function/grant
matrix end to end, and audit module-boundary/dependency direction
(generic Inventory Core must not depend on business-domain logic).

**Nine forward migrations, all live-verified**:

1. `inventory_cancel_movement` — added actor-identity (`28000`) +
   permission (`42501`, surfaced as the non-leaking `P0002` "not found
   or not accessible" message) checks; `anon`/`PUBLIC` EXECUTE revoked.
   **CRITICAL**: pre-fix, this function was `SECURITY DEFINER`
   (bypasses RLS), carried live `anon` EXECUTE, and had ZERO checks of
   any kind — any fully unauthenticated caller could cancel any DRAFT
   movement in ANY organization. Live-reproduced and closed.
2. `inventory_prevent_header_modification`/`inventory_prevent_line_
modification` — the posted-header/line immutability triggers
   redesigned. The session GUC (`ambra.inventory_movement_engine`) is
   removed ENTIRELY as an authorization signal — a caller-settable GUC
   was never a real security boundary (long-disclosed since before
   IC-2). Authorization is now based on the SUBSTANCE of the change:
   once posted, the ONLY value-changing transition ever permitted is
   EXACTLY the accepted reversal lifecycle (`status: posted→reversed`,
   `reversal_movement_id`/`reversed_by`/`reversed_at` from NULL to real
   values, every other column byte-identical to OLD — verified via
   `to_jsonb(NEW) - {allowed keys} = to_jsonb(OLD) - {allowed keys}`,
   robust against future column additions), AND the new `reversal_
movement_id` must genuinely double-link back to a real row with
   `original_movement_id = OLD.id` AND `movement_type_code = '900'`
   (which cannot be manually created — IC-7A's own closed contract) —
   closing the forgery surface completely (an attacker can no longer
   even fake an "already reversed" state pointing at an arbitrary id).
   Lines: no legitimate path ever updates/deletes an existing line
   post-posting (verified against both `inventory_reverse_movement`'s
   own body — only INSERTs new lines into a NEW reversal movement — and
   `inventory_finalize_posting_internal`'s own body — the only line-
   touching UPDATE happens while the header is still draft), so the
   GUC-gated exception was removed entirely — unconditional denial.
3. `inventory_guard_balance_write`/`inventory_guard_settings_write` —
   fixed a SEPARATE, MORE SEVERE, newly-discovered systemic bug: both
   used `current_setting('ambra.inventory_movement_engine', true) <>
'on'` as their own enforcement condition. When the GUC was NEVER SET
   AT ALL (the ordinary default for any raw client connection — no
   bypass trick required), `current_setting(..., true)` returns NULL;
   `NULL <> 'on'` evaluates to NULL; PL/pgSQL's `IF NULL THEN` treats
   NULL as FALSE — the guard failed OPEN, not closed. Live-reproduced:
   a fresh, zero-GUC-contact transaction's raw UPDATE of `on_hand_
quantity` succeeded pre-fix. Fixed with `COALESCE(current_setting(
...), 'off') <> 'on'`. Every legitimate canonical RPC already
   explicitly sets this GUC before writing, so this changes zero
   legitimate behavior.
4. `inventory_save_draft` — same CRITICAL unauthenticated-exploit class
   as item 1, same fix. `inventory_reconcile_balances` — a read-only
   diagnostic carrying live `anon` EXECUTE with zero checks (cross-
   tenant stock-drift read exposure); converted `LANGUAGE sql` →
   `LANGUAGE plpgsql` solely to add the actor+permission check (the
   diagnostic query itself unchanged); old 2-arg signature explicitly
   dropped after confirming zero application callers.
5. `inventory_reservations`/`_reservation_lines`/`inventory_
allocations`/`_allocation_lines`/`inventory_containers`/`_container_
lines` — new RESTRICTIVE `USING(false)/WITH CHECK(false)` policies
   for INSERT/UPDATE/DELETE, the exact pattern IC-4/IC-5 already proved
   safe. Closes IC-0's own 2026-09-15 finding (single PERMISSIVE ALL
   policy, no ownership-aware RESTRICTIVE policy — re-confirmed still
   live this phase via both `pg_policies` inspection and an empirical
   raw-DML probe as a real `operate`-permissioned actor). Containers:
   only the GENERIC (non-RepairOrder-owned) rows needed this closure —
   confirmed safe only because IC-6 already deleted the sole legacy
   direct-writer (`ambra-location-inventory.ts`'s own 4 write actions).
6. `inventory_create_reservation`/`inventory_release_reservation`/
   `inventory_create_allocation`/`inventory_release_allocation` —
   **genuinely new finding, discovered live while applying migration
   5**: all 4 were `SECURITY INVOKER` (the implicit Postgres default,
   never explicitly set), out of compliance with this project's own
   standing "SECURITY DEFINER, owner postgres" convention, and none had
   an actor-identity check. Because INVOKER functions run AS the
   calling role and are themselves subject to RLS, migration 5's own
   new RESTRICTIVE policies correctly blocked ordinary raw client
   writes but ALSO blocked these 4 RPCs' own legitimate writes (live-
   reproduced: `42501: new row violates row-level security policy` on
   the FIRST post-migration-5 reservation attempt). Fixed in the SAME
   phase (not deferred): converted all 4 to `SECURITY DEFINER` with
   actor-identity checks added; the two release functions' own cross-
   org existence-leak (different error messages for "not found" vs.
   "no permission") unified to the same non-leaking `P0002` pattern.
   Business logic byte-for-byte unchanged; same-owner `SECURITY
DEFINER` nesting confirmed safe for every real internal/external
   caller (all 6 real TypeScript call sites already passed a genuine,
   session-resolved actor id).
7. `inventory_movement_headers`/`inventory_movement_lines` — closed the
   status-blind raw INSERT gap (IC-0's own finding: an `operate`-
   permissioned actor could fabricate a `status='posted'` header) via a
   new RESTRICTIVE `status='draft'`-only INSERT policy. **Extended to
   lines this phase** — a genuinely new discovery: since the
   immutability trigger only fires on UPDATE/DELETE, a raw client could
   previously INSERT a brand-new line under an EXISTING, genuinely-
   posted `movement_id`, appearing to add quantity to a real document
   without ever tripping the immutability guard. Closed with the
   matching RESTRICTIVE policy requiring the parent header to still be
   `'draft'`.
8. `inventory_approve_count_session` — added the standard actor-
   identity check (closing a narrow audit-trail-attribution spoofing
   gap on the `approved_by` column for zero-net-variance approvals,
   which never reach the already-hardened nested `inventory_create_
draft`/`inventory_finalize_posting` calls); `anon`/`PUBLIC` EXECUTE
   explicitly revoked. Remains `SECURITY INVOKER` — its own nested
   calls into the hardened engine already provide the real boundary for
   any non-trivial (line-posting) approval.
9. Grant hygiene — `inventory_get_or_create_balance_for_update` (the
   shared internal balance-getter, `anon`/`PUBLIC` revoked, kept
   `authenticated`/`service_role`) and 8 trigger functions (`RETURNS
trigger`, not meaningfully PostgREST-RPC-callable regardless — `anon`/
   `PUBLIC` revoked for consistency, no live exploit found or possible).

**Systemic default-privilege decision — Option B (§9's own table row
updated in place)**: a live audit this phase (going further than IC-7A's
own deferred investigation) found dozens of genuinely, deliberately
public-facing functions across UNRELATED modules (invitation
acceptance, signup, org creation, plus dozens more across warehouse
layouts/planning/CRM/helpdesk) that legitimately rely on the same
schema-wide `ALTER DEFAULT PRIVILEGES ... GRANT EXECUTE ... TO anon`
rule — confirming changing it schema-wide is genuinely unsafe within
this phase's own Inventory-only scope. Every Inventory Core function
this phase touched received an explicit per-function `REVOKE`; the new
pgTAP file's own grant-privilege assertions plus the full function-
grant-matrix sweep serve as the enforced, re-runnable regression check
going forward, rather than relying on human migration discipline alone.

**Module-boundary audit performed** (§3/§4/§25 of the assigning brief):
generic Inventory Core does NOT depend on RepairOrder or PurchaseOrder
business logic in any load-bearing way, with ONE narrow, disclosed,
non-security-relevant exception: `inventory_add_to_container`'s own
inline RepairOrder-ownership check (a real, direct `JOIN repair_order_
lines`, guarding a real tested invariant — a container can't mix two
different RepairOrders' stock). Not touched this phase (no security
bypass involved), recorded as an architecture-compression candidate.
The TypeScript-layer dependency direction is already fully correct: a
repo-wide grep found ZERO `.rpc("inventory_...")` calls anywhere
outside `src/server/services/`, and `RepairOrdersService` (business
module) correctly calls generic Inventory Core RPCs, never the reverse.

**Application entry-point audit**: no duplicate entry points found for
the same physical write (the apparent overlap between `Inventory
EnterpriseService`'s generic reservation/allocation methods and
`RepairOrdersService`'s own line-specific methods is two DISTINCT
business operations correctly sharing one canonical RPC, not
duplication). Three canonical RPCs (`receive_repair_order_stock`,
`putaway_repair_order_stock`, `rebuild_repair_order_location_
projection`) have zero current TypeScript callers — fully built, fully
hardened, but awaiting their own UI; disclosed as an application-
completeness observation, not a security or architecture finding.

**New pgTAP file**: `110_ic7_security_write_boundary_test.sql` —
`plan(25)`, final 25/25, 0 failures. Covers the posted-header GUC
bypass (before/after, structural not GUC-state-dependent), forbidden
header UPDATE, allowed reversal transition, line-mutation denial,
`inventory_cancel_movement`'s wrong-actor/no-permission/anon/spoof/
NULL-actor sweep, reservation/allocation/container raw-write denial,
internal-helper EXECUTE denial, and every legitimate canonical path
(movement, reservation, allocation, reversal) still succeeding
unchanged.

**Full regression**: 097 through 110 (14 files) re-run — see `docs/
inventory/reviews/ic-7-review/test-evidence.md` for the complete
per-file result.

**No business semantics changed.** Every fix is additive (RESTRICTIVE
policies, actor/permission checks, grant revocations) or a pure
security-boundary redesign that preserves the exact prior legitimate
behavior while closing illegitimate paths. No new locking/concurrency
behavior was introduced.

**Full evidence**: `docs/inventory/reviews/ic-7-review/`.
