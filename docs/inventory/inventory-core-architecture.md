...
| --- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 1   | `on_hand_quantity >= 0`                                                                                                                                                                                                                      | **CHECK** (add if not already present — not confirmed live during the audit; verify in IC-0)                                                                                                         |
| 2   | `reserved_quantity >= 0`                                                                                                                                                                                                                     | **CHECK**                                                                                                                                                                                            |
| 3   | `allocated_quantity >= 0`                                                                                                                                                                                                                    | **CHECK**                                                                                                                                                                                            |
...
| 6   | No physical movement may strand a reserved/allocated commitment (i.e. a movement that would push `on_hand < reserved` or `on_hand < allocated` at that bucket is rejected, UNLESS it is a sanctioned atomic commitment-relocation operation) | **RPC transaction logic**, NEW — the core engine itself (IC-1), not a per-caller convention                                                                                                          |
...
```
available_to_move (generic physical movement) = on_hand_quantity - allocated_quantity
```

A generic movement request that would reduce `on_hand_quantity` below
`allocated_quantity` at that bucket is **REJECTED** by the core engine itself
(invariant #6). Note this is deliberately `- allocated`, not `- reserved`:
reserved-but-not-allocated stock is still "available to move" in the sense
that no specific allocation has yet claimed a physical placement for it —
moving it only requires that the _reservation_ itself be re-pointed at the new
location (an operation the reservation domain, not the movement domain, must
eventually own if this becomes a real requirement; out of scope until a
concrete business need arises — **do not build reserved-stock relocation
speculatively**).
