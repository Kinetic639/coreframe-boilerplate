...
## Phase 7 — Header, advisor ownership, lifecycle, and manual RepairOrder creation
...
- [ ] Add `createRepairOrder` service method (manual header creation): org/branch from caller's active context, optional VIN/advisor/dealer fields, `identity_status = 'unresolved'` by default (no `zl_number` at manual-creation time), enforces the same RLS INSERT policy as materialization.
- [ ] Add `assignAdvisor`/`updateHeader` service methods implementing the ownership rule: `current_user_id = crm_contacts.linked_user_id AND has_branch_permission(..., 'workshop.repair_orders.manage_own')` OR `has_branch_permission(..., 'workshop.repair_orders.manage_all')`.
- [ ] Add corresponding server actions with permission checks (defense in depth — RLS is not the only gate per the work order's explicit instruction).
- [ ] Build manual-creation UI (form: branch context implicit, VIN optional, advisor optional).
- [ ] Build header UI section showing `zl_number`, `order_number`, VIN, advisor, status, dealer/brand fields.
- [ ] Build advisor-assignment UI (contact picker scoped to `employee`-role `crm_contacts`).
- [ ] Implement status/lifecycle transitions: **`open` → `closed` → `archived`** (corrected 2026-09-10 — see Phase 2 forward migration; no `in_progress`/`cancelled`/`completed` states, RepairOrder lifecycle is intentionally simple). `archived` requires the stronger `manage_all`-equivalent/archive-capable permission per the accepted architecture — not available to a plain `manage_own` advisor. **RLS-layer enforcement of this already done ahead of schedule** (`20260910074716_repair_orders_archive_rls_restriction.sql` — `repair_orders_update`'s `WITH CHECK` now rejects any `manage_own`-only write producing `status='archived'`, live-verified via pgTAP); this task is the remaining application-layer piece (UI, server action, `canTransitionRepairOrderStatus` wiring).
- [ ] Unit tests for ownership-rule logic and for manual-creation identity-status defaulting.
- [ ] Component tests for header/advisor/manual-creation UI.
- [ ] DB/RLS test: advisor with only `manage_own` cannot edit an order they are not assigned to; advisor with `manage_all` can edit any in-branch order; advisor ownership does not grant any `inventory.*` capability (explicit negative test); archiving requires the archive-capable permission, not plain `manage_own`.

### Testing requirements

- **Unit**: ownership-rule evaluation, manual-creation identity-status defaulting.
- **Component**: header/advisor/manual-creation UI.
- **DB/RLS**: ownership boundary tests, explicit negative inventory-permission test, archive-permission boundary test.

### Acceptance criteria

A RepairOrder can be created manually (no Matcher session involved) and persists with correct org/branch/`unresolved` identity; advisor assignment persists via `advisor_contact_id`; ownership rule enforced both server-side and via RLS; advisor ownership proven (via a failing-permission test) to not grant warehouse permissions; `open`/`closed`/`archived` transitions work and `archived` is permission-gated correctly.
