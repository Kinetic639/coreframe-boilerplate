...
**Classification:** **PITCH BLOCKER (the part/container QR scope decision itself — must be explicitly resolved, either build a minimal QR target type or narrow the pitch script's "scan the part/kit" language, before rehearsal)** / PITCH SHOULD (fresh manual re-verification of the location QR path, given 6 weeks of backend churn including IC-8/A7 container-ownership changes) / PILOT (missing `qr_codes`/`qr_assignments` migrations).
...
**Classification:** PITCH SHOULD (3 small, stage-visible bugs: product search matches name only, not SKU; movement-kind label never renders "transfer" for 801/311 due to a raw-code vs. label string comparison bug; acting user never shown in history) / PITCH DECISION NEEDED (container relocation UI, or narrow the script to single-part relocation only) / PILOT (server-side pagination, concurrency/idempotency hardening).
...
**Classification: PITCH BLOCKER.** This is flagged by the research agent as the single most schedule-risky finding across all 11 zones, given the imminent "this week" pitch deadline: it requires BOTH a scope decision (which has not started) AND implementation (which has not started), and it is unavoidably visible at the very start of every demo run-through.
