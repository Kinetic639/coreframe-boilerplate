> Historical/split zone.
> Former Zone 14 ("Cykliczność i powiadomienia operacyjne") was split into two active successors: recurrence/scheduling ownership moved to [Zone 13 — Planning & Work Organization](../../zones/13-planning.md), and notifications/alerts ownership moved to a new active zone, [Zone 20 — Notifications & Operational Alerts](../../zones/20-notifications-operational-alerts.md).
> This file is preserved unchanged as the original, combined accepted audit — the authoritative historical evidence for both successor areas where their findings could not be safely separated without rewriting. It is not an active source of current pitch/pilot readiness requirements; see the two active zones above for current ownership.
