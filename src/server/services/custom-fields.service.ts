...
type ProductCustomFieldDefinition =
  Database["public"]["Tables"]["product_custom_field_definitions"]["Row"];
type ProductCustomFieldValue = Database["public"]["Tables"]["product_custom_field_values"]["Row"];

export interface ProductFieldValueWithDefinition extends ProductCustomFieldValue {
...
export class CustomFieldsService {
   ...
   */
  static async deleteFieldDefinition(
    supabase: SupabaseClient<Database>,
    definitionId: string
  ): Promise<void> {
   ...
   */
  static async reorderFieldDefinitions(
    supabase: SupabaseClient<Database>,
    updates: ReorderFieldDefinitionsInput
  ): Promise<void> {
    const promises = updates.map(({ id, display_order }) =>
      supabase.from("product_custom_field_definitions").update({ display_order }).eq("id", id)
    );
   ...
   */
  static async getProductFieldValues(
    supabase: SupabaseClient<Database>,
    productId: string
   ...
   */
  static async getVariantFieldValues(
    supabase: SupabaseClient<Database>,
    variantId: string
  ): Promise<ProductCustomFieldValue[]> {
    const { data, error } = await supabase
  ...
  static async setFieldValue(
    ...
    const { data: fieldValue, error } = await supabase
      .from("product_custom_field_values")
      .upsert(insertData, {
   ...
   */
  static async getProductFieldValuesWithDefinitions(
    supabase: SupabaseClient<Database>,
    productId: string
  ): Promise<any[]> {
    const { data, error } = await supabase
