...
export default function EditTemplatePage() {
      ...
      try {
        ...
        if (data.success && data.template) {
          ...
          setFormData({
            ...
            label_type: template.label_type || "location",
            category: template.category || "custom",
            width_mm: template.width_mm || 50,
            height_mm: template.height_mm || 25,
            dpi: template.dpi || 300,
            qr_position: template.qr_position || "center",
            qr_size_mm: template.qr_size_mm || 15,
            show_label_text: template.show_label_text ?? true,
            label_text_position: template.label_text_position || "bottom",
            label_text_size: template.label_text_size || 12,
            show_code: template.show_code ?? false,
            layout_direction: template.layout_direction || "column",
            section_balance: template.section_balance || "equal",
            background_color: template.background_color || "#FFFFFF",
            text_color: template.text_color || "#000000",
            border_enabled: template.border_enabled ?? true,
            border_width: template.border_width || 0.5,
            border_color: template.border_color || "#000000",
            is_default: template.is_default ?? false,
          });
        } else {
          toast.error("Błąd podczas ładowania szablonu", {
            description: data.error || "Szablon nie został znaleziony",
          });
  ...
  useEffect(() => {
    if (!formData.name) return; // Don't generate preview until template is loaded
