...
import type { NewsFormData } from "@/components/news/NewsForm";
...
export async function createNewsPost(formData: NewsFormData) {
  try {
    const supabase = await createClient();
    ...
    revalidatePath("/dashboard-old/news");
...
export async function updateNewsPost(newsId: string, formData: NewsFormData) {
  try {
    const supabase = await createClient();
    const appContext = await loadAppContextServer();
...
export async function getNewsPost(newsId: string) {
  try {
    const supabase = await createClient();
    const appContext = await loadAppContextServer();
