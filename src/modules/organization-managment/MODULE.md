...
- **Primary workflows:**
  ...
  - Create, update, delete custom roles (with permission assignment)
...
- [x] No raw permission strings in TypeScript — all slugs imported from `@/lib/constants/permissions` (architecture pass 2026-02-26; fixed: `roles-client.tsx`, `branches-client.tsx`, `users/branches/branches-client.tsx`, `positions-client.tsx`, `members-client.tsx`, `invitations-client.tsx`; grep evidence: 0 matches for `can\("(org|members|invites|branches)\.` in org dashboard TSX files)
...
| `organization_members`        | **SELECT**: legacy policy `"Users can view organization members"` — `(user_id = auth.uid()) OR is_org_creator(org_id) OR has_any_org_role(org_id)`. `has_any_org_role` queries `user_role_assignments` (scope='org', deleted_at IS NULL) — it does **not** check `organization_members.status` or `organization_members.deleted_at`. Result: inactive members and soft-deleted members who still have a live role assignment can read the member list for the same org. No cross-tenant risk; no mutation capability. **Consistency gap**: `removeMember()` sets `organization_members.deleted_at` but does not clean up `user_role_assignments` — removed members retain read access until roles are separately removed. Pre-V2 policy; not replaced (additive-only migration rule). **UPDATE (V2 additive)**: `is_org_member(organization_id) AND has_permission('members.manage')`.                                                                                                                                                                                                                                                                                                                                                                                                                |
...
- `20260227100000_force_rls_org_tables.sql` — FORCE ROW LEVEL SECURITY on organization_profiles, invitations, org_positions, org_position_assignments, branches (the remaining 5; all 9 module tables now have force_rls=true including role_permissions)
...
| `createRoleAction`               | `roles.ts`       | `{ name, description?, permission_slugs? }`          | ✅ `MEMBERS_MANAGE`  | ✅                   |
...
| `assignRoleToUserAction`         | `roles.ts`       | `{ userId, roleId }`                                 | ✅ `MEMBERS_MANAGE`  | ✅                   |
| `removeRoleFromUserAction`       | `roles.ts`       | `{ userId, roleId }`                                 | ✅ `MEMBERS_MANAGE`  | ✅                   |
...
- `src/server/services/organization.service.ts` — all service classes:
  ...
  - `OrgMembersService` — `listMembers`, `updateMemberStatus`, `removeMember`
  - `OrgInvitationsService` — `listInvitations`, `createInvitation`, `cancelInvitation`, `resendInvitation`
  - `OrgRolesService` — `listRoles`, `createRole`, `updateRole`, `deleteRole`, `assignRoleToUser`, `removeRoleFromUser`, `getUserRoleAssignments`
...
- Client components:
  ...
  - `organization/users/members/_components/members-client.tsx` — member list + activate/deactivate/remove, role management dialog, position assignment dialog
  - `organization/users/invitations/_components/invitations-client.tsx` — invitation list + create/resend/cancel dialog
  - `organization/users/roles/_components/roles-client.tsx` — role list + create/edit/delete dialog with permission selector (uses imported constants, no raw strings)
...
- **T1 — RLS behavior tests:**
  - `src/server/services/__tests__/organization-rls.test.ts` — 23 tests
...
- **Frontend component tests (RTL + Vitest):**
  - `src/app/[locale]/dashboard/organization/users/roles/__tests__/roles-client.test.tsx` — 5 tests
  ...
  - `src/app/[locale]/dashboard/organization/users/members/__tests__/members-client.test.tsx` — 3 tests
...
- [ ] Member role assignment (Manage Roles dialog)
- [ ] Member position assignment (Assign Position dialog)
- [ ] Invitation create → accept → member appears
...
| `users/members/page.tsx`     | `initialMembers`, `initialPositions`, `initialAssignments`, `initialRoles` | ❌ none                 |
...
| 7   | INFO     | `organization_members` SELECT: `has_any_org_role` does not check member status or deleted_at — inactive/removed members with live role assignments can read the member list. Consistency gap: `removeMember()` does not clean up `user_role_assignments`. | ⏳ Open — no cross-tenant risk, no mutation capability. Options: (a) add role cleanup in `removeMember()`, (b) restrict SELECT policy to `is_org_member`, (c) accept as-is (business-rule nuance). Pending decision.                                                                                         |
...
- `supabase/migrations/20260226100000_enable_rls_organization_profiles.sql`
- `supabase/migrations/20260226100001_org_members_v2_permission_policy.sql`
- `supabase/migrations/20260226100002_org_positions_tables.sql`
- `supabase/migrations/20260226200000_fix_invitations_rls_auth_users.sql`
- `supabase/migrations/20260226200001_create_org_logos_storage_bucket.sql`
- `supabase/migrations/20260227100000_force_rls_org_tables.sql`
- `supabase/migrations/20260227200000_fix_invitations_update_self_cancel.sql`
