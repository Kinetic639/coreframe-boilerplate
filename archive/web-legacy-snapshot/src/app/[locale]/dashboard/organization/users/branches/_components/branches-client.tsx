...
export function BranchesClient({ initialBranches }: BranchesClientProps) {
  ...
  // Called after mutations to re-sync local state
  const load = () => {
    startTransition(async () => {
      const result = await listBranchesAction();
      if ("data" in result) {
        setBranches(result.data as OrgBranch[]);
      } else {
        setError("error" in result ? (result as { error: string }).error : "Unexpected error");
      }
    });
  };
    ...
    startTransition(async () => {
      if (dialogMode === "create") {
        ...
        if (result.success) {
          ...
          toast.error("error" in result ? (result as { error: string }).error : "Unexpected error");
        }
  ...
  const handleDelete = (branch: OrgBranch) => {
    startTransition(async () => {
      const result = await deleteBranchAction({ branchId: branch.id });
      if (result.success) {
