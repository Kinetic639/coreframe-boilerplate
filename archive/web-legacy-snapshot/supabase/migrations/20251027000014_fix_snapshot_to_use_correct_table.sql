-- =============================================
-- Fix refresh_stock_snapshot to use stock_snapshots table (not product_location_stock)
-- =============================================
...
BEGIN
  -- Check if stock_snapshots table exists
  SELECT EXISTS (
  ...
  -- Upsert snapshot into stock_snapshots table (not product_location_stock)
...
'Refreshes the stock snapshot in stock_snapshots table for a product/variant at a specific location.
...
DO $$
BEGIN
  RAISE NOTICE 'refresh_stock_snapshot function updated to use stock_snapshots table';
  RAISE NOTICE 'Function uses proper source_location_id and destination_location_id handling';
END $$;
